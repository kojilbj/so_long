...
/*   Updated: 2023/12/12 16:00:46 by kojwatan         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	reflesh_window(int direction, t_vars vars)
{
	...
	if (direction == UP)
	{
		mlx_put_image_to_window(vars.mlx, vars.win, vars.map_imgs.player, x
			* PANEL_SIZE, y * PANEL_SIZE);
		mlx_put_image_to_window(vars.mlx, vars.win, vars.map_imgs.background, x
			* PANEL_SIZE, (y + 1) * PANEL_SIZE);
	}
	if (direction == DOWN)
	{
		mlx_put_image_to_window(vars.mlx, vars.win, vars.map_imgs.player, x
...
int	move_player_check(int keycode, t_vars *vars)
{
	if (keycode == 119 || keycode == 65362)
	{
		if (vars->map_info.map[vars->player_info.curr_y
				- 1][vars->player_info.curr_x].texture != '1')
			return (UP);
	}
